...
flash = Token:match("(%d+)")
redis:set(flash..":VERSION",1)
redis:set(flash..":SUDO_ID:",SUDO_USER)
redis:set(flash..":DataCenter:",'German')
redis:set(flash..":UserNameBot:",BOT_User)
redis:set(flash..":NameBot:",BOT_NAME)
redis:hset(flash..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3flash_INSTALL","Yes")
info = {}
...
flash = Token:match("(%d+)")
our_id = tonumber(flash)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(flash..":UserNameBot:")
SUDO_ID = tonumber(redis:get(flash..":SUDO_ID:"))
SUDO_USER = redis:hgetall(flash..'username:'..SUDO_ID).username
version = redis:get(flash..":VERSION")
DataCenter = redis:get(flash..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..flash..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(flash..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(flash..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(flash..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif msg.sender_user_id_ == 11111111146889000 then 
...
msg.Rank = 1
elseif redis:sismember(flash..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(flash..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(flash..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(flash..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(flash..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(flash..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(flash..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(flash..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(flash..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(flash..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(flash..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(flash..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس فلاش ⇐ @SUORSFLASH
...
if msg.GroupActive and redis:get(flash..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(flash..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(flash..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(flash..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.flash then 
if msg.text and ScriptFile.iflash then
for k, flash in pairs(ScriptFile.flash) do
local SearchText = msg.text:match(flash)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iflash(msg,{msg.text:match(flash)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",flash.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dflash then
if ScriptFile.dflash(msg) == false then
return false
end
...
if Plug.flash then 
if msg.text and Plug.iflash then
for k, flash in pairs(Plug.flash) do
local SearchText = msg.text:match(flash)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(flash..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(flash..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iflash(msg,{msg.text:match(flash)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",flash.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source flash")
end 

end
else
print("The File Script.lua Not Runing in The Source flash")
...
function tdcli_update_callback(data)
	...
	redis:srem(flash..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/flash/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/flash/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/flash/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/flash/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/flash/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/flash/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(flash..":SUDO_ID:")
redis:hset(flash..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(flash..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(flash..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(flash..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(flash..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(flash..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(flash..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(flash..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(flash..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(flash..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(flash..'group:add'..data.chat_.id_) then
	redis:set(flash..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(flash..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(flash..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(flash..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	flash..'group:name'..k,IDS.Title,
	flash..'num_msg_flash'..k,5,
	flash..'group:add'..k,true,
	flash..'lock_link'..k,true,
	flash..'lock_spam'..k,true,
	flash..'lock_webpage'..k,true,
	flash..'lock_markdown'..k,true,
	flash..'lock_flood'..k,true,
	flash..'lock_bots'..k,true,
	flash..'mute_forward'..k,true,
	flash..'mute_contact'..k,true,
	flash..'mute_document'..k,true,
	flash..'mute_inline'..k,true,
	flash..'lock_username'..k,true,
	flash..'replay'..k,true
	)
	redis:sadd(flash..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(flash..'username:'..ID,'username',user)
	redis:sadd(flash..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(flash..'username:'..ID,'username',user)
	redis:sadd(flash..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(flash..'username:'..ID,'username',user)
	redis:sadd(flash..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(flash..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(flash..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(flash..'username:'..data.user_.id_).username
	...
	redis:hset(flash..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
